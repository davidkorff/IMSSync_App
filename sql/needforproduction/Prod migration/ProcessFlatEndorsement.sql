-- =============================================
-- FINAL Flat Endorsement Procedure V8 - PRODUCTION READY
-- Simplified version without fee/charge copying
-- =============================================
...
EXEC [dbo].[ProcessFlatEndorsement]
    @OriginalQuoteGuid = '2C5C0423-BFE4-4759-B288-14DF77AD4D3C',
    @NewPremium = 5000,
    @EndorsementEffectiveDate = GETDATE(),
    @EndorsementComment = 'Flat premium adjustment',
    @UserGuid = NULL,
...
-- Check the new endorsement quote
IF @NewQuoteGuid IS NOT NULL
BEGIN
    SELECT 'New Endorsement Quote' AS QueryType, * 
    FROM tblQuotes 
    WHERE QuoteGuid = @NewQuoteGuid;
    
    SELECT 'Quote Option' AS QueryType, * 
    FROM tblQuoteOptions 
    WHERE QuoteGuid = @NewQuoteGuid;
    
    SELECT 'Premiums' AS QueryType, qop.* 
    FROM tblQuoteOptions qo
    INNER JOIN tblQuoteOptionPremiums qop ON qo.QuoteOptionGuid = qop.QuoteOptionGuid
    WHERE qo.QuoteGuid = @NewQuoteGuid;
END

call with EXEC ProcessFlatEndorsement
      @OriginalQuoteGuid = '2C5C0423-BFE4-4759-B288-14DF77AD4D3C',
      @NewPremium = 5000.00,
      @EndorsementEffectiveDate = '2025-12-20'
*/