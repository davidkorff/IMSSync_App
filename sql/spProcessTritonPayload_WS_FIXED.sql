...
BEGIN
    ...
    BEGIN TRY
        ...
        -- DEBUG: Log extracted market_segment_code
        PRINT '===== ProgramID Assignment Debug Info =====';
        PRINT 'Transaction Type: ' + ISNULL(@transaction_type, 'NULL');
        PRINT 'Extracted market_segment_code: ' + ISNULL(@market_segment_code, 'NULL');
        ...
        -- 2. Insert or update tblTritonQuoteData with ALL fields including market_segment_code
        -- Check if quote already exists
        IF EXISTS (SELECT 1 FROM tblTritonQuoteData WHERE QuoteGuid = @QuoteGuid)
        BEGIN
            -- Update existing record with latest data (including market_segment_code)
        ...
        ELSE
        BEGIN
            -- Insert new record (including market_segment_code)
        ...
        BEGIN
            ...
            -- DEBUG: Log the CompanyLineGuid
            PRINT 'Retrieved CompanyLineGuid from tblQuotes: ' + ISNULL(CAST(@CompanyLineGuid AS NVARCHAR(50)), 'NULL');
            
            -- Check if tblQuoteDetails exists for this quote
            ...
            ELSE
            BEGIN
                PRINT 'WARNING: tblQuoteDetails record DOES NOT EXIST for this quote - cannot set ProgramID';
            END
           
            -- Set ProgramID based on market segment and line combinations
            IF @QuoteDetailsExists = 1
            BEGIN
                ...
                BEGIN
                    ...
                    SET ProgramID = 11615
                    WHERE QuoteGuid = @QuoteGuid;
                    PRINT 'MATCHED: Set ProgramID to 11615 (RT market segment, Primary LineGuid 07564291)';
                END
                ...
                BEGIN
                    ...
                    SET ProgramID = 11613
                    WHERE QuoteGuid = @QuoteGuid;
                    PRINT 'MATCHED: Set ProgramID to 11613 (WL market segment, Primary LineGuid 07564291)';
                END
                ...
                BEGIN
                    ...
                    SET ProgramID = 11612
                    WHERE QuoteGuid = @QuoteGuid;
                    PRINT 'MATCHED: Set ProgramID to 11612 (RT market segment, Excess LineGuid 08798559)';
                END
                ...
                BEGIN
                    ...
                    SET ProgramID = 11614
                    WHERE QuoteGuid = @QuoteGuid;
                    PRINT 'MATCHED: Set ProgramID to 11614 (WL market segment, Excess LineGuid 08798559)';
                END
                ELSE
                BEGIN
                    PRINT 'NO MATCH: ProgramID rule not found for:';
                ...
                -- Verify the update
                SELECT @CurrentProgramID = ProgramID FROM tblQuoteDetails WHERE QuoteGuid = @QuoteGuid;
                PRINT 'Final ProgramID in tblQuoteDetails: ' + ISNULL(CAST(@CurrentProgramID AS NVARCHAR(10)), 'NULL');
            END
        END
        ELSE
        BEGIN
            PRINT 'Skipping ProgramID assignment - transaction_type is: ' + @transaction_type;
        END
