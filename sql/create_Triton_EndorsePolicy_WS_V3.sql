...
--              V3: Removed premium insertion - let spProcessTritonPayload handle it
--                  to avoid double premium application
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Step 3: REMOVED - Premium insertion removed to avoid double application
        -- Premium will be applied via spProcessTritonPayload which calls UpdatePremiumHistoricV3
        -- This ensures proper premium handling and avoids the "Can't modify premiums on a bound policy" error
        ...
        -- Step 6: Bind the endorsement if requested
        IF @BindEndorsement = 1 AND @EndorsementQuoteGuid IS NOT NULL
        BEGIN
            -- Update DateBound
            UPDATE tblQuotes
            SET DateBound = GETDATE()
            ...
            -- Mark the quote option as bound
        ...
        -- Return result set similar to Ascot's approach
        SELECT 
            @Result AS Result,
            @Message AS Message,
