USE [Greyhawk_Test]
GO
/****** Object:  StoredProcedure [dbo].[Ascot_CancelPolicy]    Script Date: 7/17/2025 10:24:14 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[Ascot_CancelPolicy]
(
...
BEGIN
	...
	FOR SELECT	NewQuoteOptions.QuoteOptionGuid,
				...
				THEN
					CASE WHEN tblFin_PolicyCharges.ChargeID = 'PREM' THEN @NewPremium
					WHEN tblFin_PolicyCharges.ChargeID = 'TERR' THEN @TerrorismPremium
					WHEN tblFin_PolicyCharges.ChargeID = 'FACU' THEN @PrifacPrem
				END
				ELSE
					CASE WHEN ChargeCodeTable.ChargeID = 'PREM' THEN NewPremiumTable.Premium 
					WHEN ChargeCodeTable.ChargeID = 'TERR' THEN NewPremiumTable.Terrorism 
					WHEN tblFin_PolicyCharges.ChargeID = 'FACU' THEN '0.0'
				END
		...
		OUTER APPLY (
						SELECT	ISNULL(Ascot_AL3QuoteDetailTable.Premium, 0) AS Premium,
								ISNULL(Ascot_AL3QuoteDetailTable.Terrorism, 0) AS Terrorism,
								Ascot_AL3QuoteDetailTable.CompanyLocationCode,
								Ascot_AL3QuoteDetailTable.LineCode,
								Ascot_AL3QuoteDetailTable.StateID
						FROM	Ascot_AL3QuoteDetailTable
						WHERE	Ascot_AL3QuoteDetailTable.DnlTrId = @DnlTrID
					) AS NewPremiumTable
		OUTER APPLY (
		...
		OUTER APPLY (
						SELECT Ascot_AL3PolicyMasterTable.CompanyCode AS CompanyLocationCode,
								Ascot_AL3PolicyMasterTable.LineCode
						FROM Ascot_AL3PolicyMasterTable
						WHERE Ascot_AL3PolicyMasterTable.DnlTrId = @DnlTrId
	...
	BEGIN
		...
		SELECT	@BrokerCommissionPercent = Ascot_AL3PolicyMasterTable.BrokerCommissionPercent,
				@BrokerFeeAppliesToLineGuid = lstLines.LineGUID,
				@BrokerFeeAppliesToCompanyLineGuid = CL.CompanyLineGuid
		FROM Ascot_AL3PolicyMasterTable
		INNER JOIN lstLines ON Ascot_AL3PolicyMasterTable.LineCode = lstLines.LineID
		Inner Join  tblCompanyLocations on tblCompanyLocations.CompanyLocationCode = Ascot_AL3PolicyMasterTable.CompanyCode
		Inner Join  tblCompanyLines CL on CL.LineGUID = lstLines.LineGUID and CL.CompanyLocationGUID = tblCompanyLocations.CompanyLocationGUID and Ascot_AL3PolicyMasterTable.StateOfIssuance = CL.StateID
	...
	END
      ...
      DECLARE @InvoiceNum INT
		...
		FROM Ascot_AL3PolicyMasterTable P
		...
		BEGIN
			...
			SELECT @InvoiceProcessInstallmentMod = I.InstallmentMod,
					...
					@InvoiceAmount = CASE WHEN @ProcessedInvoiceCount = 0 THEN I.InstallmentMod * (@NewPremium + @TerrorismPremium + @PrifacPrem) END
	...
	--Verify Invoices were created
	  ...
	  UPDATE Ascot_AL3PolicyMasterTable SET CreatedQuoteID = @EndorsementQuoteID
	  FROM Ascot_AL3PolicyMasterTable
	  WHERE DnlTrID = @DnlTrID
