USE [Greyhawk_Test]
GO
/****** Object:  StoredProcedure [dbo].[Ascot_EndorsePolicy_V6]    Script Date: 7/17/2025 10:23:22 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[Ascot_EndorsePolicy_V6]
(
...
BEGIN
	...
	FROM ASCOT_AL3PolicyMasterTable_V2 where DnlTrId = @DnlTrID
	...
	--select 'tblQuoteOptions' as tablename, * from tblQuoteOptions  WHERE QuoteGuid = @EndorsementQuoteGuid
	...
	FOR SELECT NewQuoteOptions.QuoteOptionGuid,
              ...
              INNER JOIN    tblQuotes NewQuoteOptionsQuote ON NewQuoteOptions.QuoteGUID = NewQuoteOptionsQuote.QuoteGUID
                     AND           NewQuoteOptionsQuote.ControlNo = tblQuotes.ControlNo
              CROSS APPLY (
	...
	--		AND     tblCompanyLines.statusID = 1
	--		AND (
 --                                   (FeeLine.LineGUID = 'C715B0AB-9238-478A-A866-8A9522360EC3' 
 --                                       AND tblCompanyLines.ParentCompanyLineGUID = '868EB509-7AB1-47C5-8B02-0AEFADE54BEE')
 --                                   OR
 --                                   (FeeLine.LineGUID = '385FED9C-6611-4927-83D4-4D43D0A85B5F' 
 --                                       AND tblCompanyLines.ParentCompanyLineGUID = '7C02DBF5-648C-472E-8D09-EB08105CFE06')
 --                               )
	...
	SELECT 		t.*,
				...
				l.LineGuid
	INTO		#PolsToCreate
	FROM		Ascot_AL3PolicyMasterTable_v2 t
	INNER JOIN	lstLines l ON l.LineID=t.LineCode
	Inner Join  tblCompanyLocations on tblCompanyLocations.CompanyLocationCode = t.CompanyCode
	Inner Join  tblCompanyLines CL on CL.LineGUID = l.LineGUID and CL.CompanyLocationGUID = tblCompanyLocations.CompanyLocationGUID and t.StateOfIssuance = CL.StateID and cl.ParentCompanyLineGUID IS NULL
	...
	BEGIN
		...
		SELECT	qo.QuoteOptionGUID,
				...
				0
		FROM		#PolsToCreate P
		INNER JOIN Ascot_AL3FeesTable ON p.DnlTrId = Ascot_AL3FeesTable.DnlTrID
		INNER JOIN	lstLines FeeLine ON Ascot_AL3FeesTable.LineID = FeeLine.LineID
		INNER JOIN	tblCompanyLocations FeeCompanyLocation ON FeeCompanyLocation.CompanyLocationCode = Ascot_AL3FeesTable.CompanyLocationCode
		INNER JOIN	tblQuotes q ON q.QuoteGUID = @EndorsementQuoteGuid
		INNER JOIN	tblQuoteOptions qo ON q.QuoteGUID = qo.QuoteGUID
		...
		INNER JOIN	tblCompanyLines ON tblCompanyLines.LineGUID = FeeLine.LineGUID
			AND		tblCompanyLines.CompanyLocationGUID = FeeCompanyLocation.CompanyLocationGUID
			AND		tblCompanyLines.StateID = Ascot_AL3FeesTable.StateID
			AND     P.companyLineguid = ISNULL(tblCompanyLines.ParentCompanyLineGUID,P.CompanyLineGUID)
			AND		CASE WHEN FeeLine.LineGUID <> q.LineGUID
						THEN CASE WHEN tblCompanyLines.ParentCompanyLineGUID IS NOT NULL THEN 1 ELSE 0 END
						ELSE CASE WHEN tblCompanyLines.ParentCompanyLineGUID IS NULL THEN 1 ELSE 0 END
					END = 1
	...
	--		AND		tblCompanyLines.StateID = Ascot_AL3FeesTable.StateID
	--		AND     tblCompanyLines.statusID = 1
	--		AND     tblCompanyLines.ParentCompanyLineGUID='7C02DBF5-648C-472E-8D09-EB08105CFE06'----KMG 24/06/2025
	--		AND		CASE WHEN FeeLine.LineGUID <> q.LineGUID
	--					THEN CASE WHEN tblCompanyLines.ParentCompanyLineGUID IS NOT NULL THEN 1 ELSE 0 END
	--					ELSE CASE WHEN tblCompanyLines.ParentCompanyLineGUID IS NULL THEN 1 ELSE 0 END
	--				END = 1
	...
	BEGIN
		...
		--DECLARE @NumInstallmentsForTrue int

		SELECT @NumInstallments = ISNULL(P.NumInstallments, 1)
				--@NumInstallments = case when ISNULL(P.NumInstallments, 1) = 11 then 1
				--								 when ISNULL(P.NumInstallments, 1) = 21 then 2
				--								  when ISNULL(P.NumInstallments, 1) = 41 then 4
				--								  else ISNULL(P.NumInstallments, 1)
				--									end
		...
		(
			...
			TridentNumInstallments int
		)

					INSERT INTO @InstallmentPlanMappingTable VALUES('Annual Agency Bill True',1)
                    INSERT INTO @InstallmentPlanMappingTable VALUES('Quarterly Agency Bill True',4)
                    INSERT INTO @InstallmentPlanMappingTable VALUES('Semi Annual Agency Bill True',2)
                    --INSERT INTO @InstallmentPlanMappingTable VALUES('Annual Agency Bill',1)
                    --INSERT INTO @InstallmentPlanMappingTable VALUES('Quarterly Agency Bill',4)
                    --INSERT INTO @InstallmentPlanMappingTable VALUES('Semi Annual Agency Bill',2)
		...
		AND T.TridentNumInstallments = @NumInstallments
		...
		DECLARE @FirstInvoiceFromEffEndMonthDate bit
		...
		BEGIN
			...
			SELECT	@FirstInvoicePercentage = tblCompanyLineInstallments.DownpaymentPercentage,
					...
					@FirstInvoiceFromEffEndMonthDate = tblCompanyLineInstallments.DownPaymentFromEffEndMonth,
									
					@InstallmentPercentage = ((1.0000 - tblCompanyLineInstallments.DownpaymentPercentage) / isnull(nullif(tblCompanyLineInstallments.NumPayments,0), 1)),
					@InstallmentTerms = tblCompanyLineInstallments.PolicyEffectiveInstallmentTerm,
		...
		BEGIN
			...
			BEGIN
				...
				ELSE IF @FirstInvoiceFromEffEndMonthDate = 1
				BEGIN

					SET @CalculatedDueDate = DATEADD(d, @FirstInvoiceTerm, EOMONTH(@TransEffDate))
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SET @CalculatedDueDate = DATEADD(m, @InstallmentTerms * (@CurrentInstallmentNumber - 1), EOMONTH(@TransEffDate))

					END
					ELSE
					BEGIN
									
						SET @CalculatedDueDate = CASE WHEN @CurrentInstallmentNumber > 2  
							THEN DATEADD(d, @InstallmentTerms + 90*(@CurrentInstallmentNumber - 2), EOMONTH(@TransEffDate))
							ELSE DATEADD(d, @InstallmentTerms * (@CurrentInstallmentNumber - 1), EOMONTH(@TransEffDate))
							END
...
--		BEGIN
		...
		BEGIN
			...
			DECLARE @QuoteOptionLineGuid UniqueIdentifier

			DECLARE db_cursor CURSOR FOR  
				SELECT DISTINCT tblQuoteOptionCharges.Amount, tblQuoteOptionCharges.OptionFeeID , tblQuoteOptions.LineGuid
				FROM tblQuotes
				INNER JOIN tblQuoteOptions ON tblQuotes.QuoteGUID = tblQuoteOptions.QuoteGUID
				INNER JOIN tblQuoteOptionCharges ON tblQuoteOptions.QuoteOptionGUID = tblQuoteOptionCharges.QuoteOptionGuid
			...
			OPEN db_cursor   
			FETCH NEXT FROM db_cursor INTO @QuoteOptionChargeAmount, @QuoteOptionFeeID, @QuoteOptionLineGuid

			WHILE @@FETCH_STATUS = 0   
			BEGIN   

					IF OBJECT_ID('tempdb..#TridentLineGuid') IS NOT NULL BEGIN DROP TABLE #TridentLineGuid END
							--Declare #TridentLineGuid UniqueIdentifier
							select Lineguid into #TridentLineGuid from lstlines where linename like '%Trident%' --OR linename like '%Program - Trident%'
							--Select @TridentLineGuid = LineGuid from lstlines where linename = 'Program Trident - Auto Liability'
							
							If  @QuoteOptionLineGuid in (Select Lineguid from #TridentLineGuid)
							Begin
		...
		--Verify Premiums
		EXEC spAccountingTransfer_VerifyPremiumsTransferred_V2
					@EndorsementQuoteGuid, --@quoteGuid
					0, --@debug
					0 --@reVerify
