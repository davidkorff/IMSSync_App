USE [Greyhawk_Test]
GO
/****** Object:  StoredProcedure [dbo].[Ascot_ReinstatePolicy_Captive]    Script Date: 7/17/2025 10:16:49 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[Ascot_ReinstatePolicy_Captive]
(
...
BEGIN
	...
	IF (EXISTS(SELECT * FROM tblQuotes WHERE QuoteGuid = @QuoteGuid AND DateBound IS NULL))
	BEGIN
		RAISERROR('Quote Must Be In A Bound Status.',15,1)
	END

	IF (NOT EXISTS(SELECT * FROM lstQuoteStatusReasons where QuoteStatusID = 8 AND ID = @QuoteStatusReasonID))
	BEGIN
	...
	EXEC  spCopyQuote @QuoteGuid = @QuoteGuid,
						@TransactionTypeID = 'R',
						@QuoteStatusID = 8,
	...
	----Update the expiration date of the original quote
	--UPDATE
	--	tblQuotes
	--SET
	--	ExpirationDate = (SELECT TOP 1 ExpirationDate FROM tblQuotes WITH (NOLOCK)
	--					  WHERE ControlNo=@ControlNumber ORDER BY QuoteID ASC)
	--WHERE
	--	QuoteGuid = @QuoteGuid

    --At this point, the endorsement has been created.  We now
    --want to copy forward the old premium record with the new amount
	...
	WHERE DnlTrID = @DnlTrID
	 ...
	 -- --we need to reset the expiration date
	 -- --get the current quoteid, controlno, and expiration date
	 -- DECLARE @ControlNo int
	 -- DECLARE @CurrentExpirationDate datetime
	 -- DECLARE @QuoteID int
	 -- SELECT @ControlNo=ControlNo, @CurrentExpirationDate=dbo.DateOnly(ExpirationDate), @QuoteID=QuoteID FROM tblQuotes WHERE QuoteGuid=@EndorsementQuoteGuid
	
	 -- --find the first original expiration date if set on previous transactions
	 -- DECLARE @oQuoteID int
	 -- SELECT @oQuoteID=MAX(q2.QuoteID) FROM tblQuotes q INNER JOIN tblQuotes2 q2 ON q.QuoteID=q2.QuoteID WHERE ControlNo=@ControlNo AND q2.QuoteID<@QuoteID AND dbo.DateOnly(q2.OriginalExpirationDate)<>dbo.DateOnly(@CurrentExpirationDate) AND q2.OriginalExpirationDate IS NOT NULL
	  
	 -- --if that value is not set use the expiration date from the original bind
	 -- DECLARE @OriginalExpDate datetime
	 -- IF @oQuoteID IS NULL
		--SELECT @OriginalExpDate=dbo.DateOnly(ExpirationDate) FROM tblQuotes WHERE ControlNo=@ControlNo AND OriginalQuoteGuid IS NULL
	 -- ELSE
		--SELECT @OriginalExpDate=dbo.DateOnly(OriginalExpirationDate) FROM tblQuotes2 WHERE QuoteID=@oQuoteID

	 -- IF @OriginalExpDate IS NOT NULL	
		--UPDATE tblQuotes SET ExpirationDate=@OriginalExpDate WHERE QuoteGuid=@EndorsementQuoteGuid
