"""
Test 3: New Business Bind Workflow - Fixed version
        ...
        if json_file:
            ...
            try:
                with open(json_file, 'r') as f:
                    payload = json.load(f)
                print(f"✓ Successfully loaded payload from {json_file}")  # Use print as backup
                logger.info(f"✓ Loaded payload from {json_file}")
            except Exception as e:
                print(f"✗ Failed to load JSON: {e}")  # Use print for errors too
...
if __name__ == "__main__":
    ...
    print(f"Starting test with json={args.json}")  # Debug line
